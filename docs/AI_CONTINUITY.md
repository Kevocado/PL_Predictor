...
| Sofascore / FotMob live APIs | Live match possession, momentum, and other rich stats | Not recommended without an explicit, conscious decision — same category as `pulselive.py` | Both are undocumented, unofficial private APIs (like pulselive.com) with no published terms; both blocked outright (403/404) from this project's development environment on a first check (2026-08-25), unconfirmed whether that reflects a real access restriction or just this environment. Even if reachable, this needs the same explicit user sign-off pulselive.com already required — never wire in an unofficial feed silently. |
