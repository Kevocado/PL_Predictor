...
function OverallCell({ player }: { player: PlayerHubPlayer }) {
  if (player.rating_status !== "established") {
    ...
    return <td className="px-1.5 py-2 text-right"><span className="inline-flex rounded-md border border-pl-pink/50 bg-pl-pink/10 px-2 py-1 text-xs font-bold text-pl-pink">{label}</span></td>;
  }
    ...
    <section className="flex flex-col gap-6">
      <div className="mb-3 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-pl-text-dim">Overall is durable Quality plus a capped current-form lift. Read Quality beside it, then Live Form and vs Quality to see who is outperforming their usual level. Provisional means fewer than 900 recent Premier League minutes; Limited evidence means only one qualifying season. Neither is rankable yet.</p>
          <p className="mt-1 text-xs text-pl-text-faint">Rated on {(data.rating_model_source ?? "season form").replaceAll("_", " ")}, {data.data_freshness ?? "cached FPL data"}</p>
        </div>
      ...
      <div className="flex flex-wrap items-end gap-3 rounded-xl border border-pl-border bg-pl-850/40 p-3 text-xs text-pl-text-dim">
        <div className="flex gap-1 rounded-lg border border-pl-border p-1">
          {["ALL", "GK", "DEF", "MID", "FWD"].map((position) => <button key={position} onClick={() => updateFilter(setPositionFilter, position)} className={`rounded-md px-2 py-1.5 font-semibold ${positionFilter === position ? "bg-pl-pink text-pr-accent-ink" : "hover:text-pl-text"}`}>{position === "ALL" ? "All positions" : position}</button>)}
        </div>
