...
function LineupCard({ data, title, requestedFormation, supplementarySummary, showBenchCards = false }: { data: FPLRecommendation; title: string; requestedFormation: string; supplementarySummary?: string; showBenchCards?: boolean }) {
  ...
  return <section className="rounded-xl border border-pl-border bg-pl-900/50 p-4">
    <div className="mb-3 flex items-start justify-between gap-2"><div><h3 className="font-semibold text-pl-text">{title}</h3><p className="text-xs text-pl-text-dim">{requestedFormation === "Auto" ? `Optimizer chose ${formation}, projected` : `${formation} selected, projected`} {data.projected_points.toFixed(1)} points before captaincy{supplementarySummary && `. ${supplementarySummary}`}</p></div><span className="rounded bg-pl-pink/15 px-2 py-1 text-xs font-bold text-pl-pink">C {data.captain?.web_name ?? "—"}</span></div>
    ...
    {showBenchCards ? (
      <div className="mt-3 rounded-lg border border-pl-border/70 bg-pl-850/30 p-2.5">
        <p className="mb-2 text-xs text-pl-text-faint">Bench order</p>
...
export function FPLPage() {
  ...
  return <div className="space-y-6">
      ...
      {(["gameweek", "squad", "transfers"] as const).map((m) => (
        <button
          ...
          className={`rounded-md px-3 py-1.5 text-xs font-semibold ${
            mode === m ? "bg-pl-pink text-pr-accent-ink" : "text-pl-text-dim hover:text-pl-text"
          }`}
        >
          {MODE_LABELS[m]}
        </button>
    ...
    {mode === "gameweek" && (
      <section><div className="mb-2 flex flex-wrap gap-1">{FORMATIONS.map((item) => <button key={item} onClick={() => chooseFormation(item)} className={`rounded-md px-2.5 py-1 text-xs font-semibold ${formation === item ? "bg-pl-pink text-pr-accent-ink" : "border border-pl-border text-pl-text-dim hover:text-pl-text"}`}>{item}</button>)}</div>{xi ? <LineupCard data={xi} title="Best starting XI" requestedFormation={formation} supplementarySummary="No budget cap, legal 15-player matchday squad" showBenchCards /> : <section className="rounded-xl border border-pl-border bg-pl-900/50 p-4 text-sm text-pl-text-faint">Optimising the best starting XI…</section>}<p className="mt-2 text-xs text-pl-text-faint">This is the highest projected XI for this gameweek, then a legal substitute bench. The £100m squad uses the same projections but must also fit the budget, so overlap is normal while differences are meaningful.</p></section>
    )}
